...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      success('Welcome back!', `Hello ${user.firstName || user.username}! You have successfully logged in.`);
      navigate('/app');
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
        <p className="text-xs text-blue-400 text-center mb-2">New to NeuroFi?</p>
        <p className="text-xs text-slate-700 dark:text-gray-300 text-center">
          Create an account to start trading with AI-powered recommendations
        </p>
      </div>
    </motion.div>
  );
